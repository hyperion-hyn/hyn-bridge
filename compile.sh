...
abigen --bin=./build/bin/ERC20AtlasManager.bin --abi=./build/abi/ERC20AtlasManager.abi --type=ERC20AtlasManager --pkg=main --out=./deploy/atlas/auto_gen_erc20_atlas_manager.go
...
abigen --bin=./build/bin/ERC20HecoManager.bin --abi=./build/abi/ERC20HecoManager.abi --type=ERC20HecoManager --pkg=main --out=./deploy/atlas/auto_gen_erc20_heco_manager.go
...
abigen --bin=./build/bin/TokenManager.bin --abi=./build/abi/TokenManager.abi --type=TokenManager --pkg=main --out=./deploy/atlas/auto_gen_token_manager.go
